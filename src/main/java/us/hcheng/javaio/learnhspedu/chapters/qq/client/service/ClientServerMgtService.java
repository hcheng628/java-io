	...
	public void run() {
		...
		while (!socket.isClosed()) {
			...
			if (type == Msg.MsgType.MSG) {
				System.out.println(msg.getFrom() + " said to " + msg.getTo() + ": " + msg.getContent());
			} else if (type == Msg.MsgType.MSG_ALL) {
				System.out.println(msg.getFrom() + " said to ALL: " + msg.getContent());
				...
				try (BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(dest))) {
					bos.write(data);
					System.out.println("Received a file: " + dest);
